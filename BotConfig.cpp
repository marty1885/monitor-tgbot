...
Config parse_and_load(int argc, char** argv)
{
	...
	system(("mkdir -p " + default_config_dir).c_str()); // C++fs seems not respect ~ for directories
	...
	if(config_file.empty()) {
		// HACK: A stupid way to check if the file exists
		FILE* f = fopen(default_config_file.c_str(), "r");
		if(f != nullptr) {
